...
* ./ast_display.odin
* This file implements::($Ast__Copy__)::and::($ast_to_string) procedure groups for copying and displaying the AST.
*/
...
// Copies the array of nodes to destination parameter.
ast_copy_array :: proc(ast: ^Ast_Array, dst: ^Ast_Array, allocator: mem.Allocator) {
	for &stmt in ast {
		append(dst, ast_copy(&stmt, allocator))
	}
}
// Copies the array of identifiers to destination parameter, creating clones of each identifier.
@(private = "file")
...
// Copies the array of blocks to destination parameter.
@(private = "file")
...
// Creates a clone of the value parameter.
...
// Procedure group for displaying an AST
ast_to_string :: proc {
...
// Main procedure for displaying an AST.
@(private = "file")
