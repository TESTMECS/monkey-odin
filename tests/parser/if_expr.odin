...
test_parse_if_expression :: proc(t: ^testing.T) {
	...
	if !ok {
		log.errorf("program[0] is not Ast_If, got='%v'", m.ast_type(program[0]))
		return
	}
	// stmt.condition
	if !literal_value_is_valid(stmt.condition, true) {
		log.errorf("stmt.condition is not 'true', got='%v'", m.ast_type(stmt.condition))
		return
	}
	...
	if !literal_value_is_valid(&stmt.then[0], 10) {
		log.errorf("stmt.then[0] is not '10', got='%v'", m.ast_type(stmt.then[0]))
		return
	}
	...
	if !literal_value_is_valid(&stmt.orelse[0], 20) {
		log.errorf("stmt.orelse[0] is not '20', got='%v'", m.ast_type(stmt.orelse[0]))
		return
	}
}
